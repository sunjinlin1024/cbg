...
var AutoComplete = function(dom, options) {
    ...
    function() {};
    this.selectAtEnter = options.selectAtEnter === void 0 ? true: options.selectAtEnter;
        ...
        if (__this.selector.visiable) {
            ...
            if (keynum == 38) {
                if (__this.selector.last_item_index < 0) {
                    ...
                    cur_index = __this.selector.last_item_index - 1 __this.dom.value = __this.selector.dom.childNodes[cur_index].autoCompleteSelectItem.name;
                }
                ...
                if (__this.selector.last_item_index != -1) __this.dom.value = __this.selector.dom.childNodes[__this.selector.last_item_index].autoCompleteSelectItem.name;
                __this.callback();
    ...
    if (window.addEventListener) {
        __this.dom.addEventListener('input',
        function() {
            if (__this.dom.value.length < __this.startPoint) __this.selector.hide();
            else {
                __this.selector.last_item_index = -1;
                __this.search();
            }
    ...
    function onMouseClick(evt) {
        ...
        if (target != __this.dom && target != __this.selector.dom) {
            if (__this.automatic && __this.selector.last_item_index != -1) __this.dom.value = __this.selector.dom.childNodes[__this.selector.last_item_index].autoCompleteSelectItem.name;
            __this.selector.hide();
        }
    }
    if (document.body.attachEvent) document.body.attachEvent('onclick', onMouseClick);
    else document.body.addEventListener('click', onMouseClick, false);
}
AutoComplete.prototype = {
    ...
    set_result: function(result) {
        if (result.length > this.promptNum) result = result.slice(0, this.promptNum);
        this.parseData.call(this, result);
        if (result.length > 0) {
            var ul = this.selector.dom;
            ul.style.width = 'auto';
            if (ul.clientWidth > this.dom.offsetWidth) ul.style.width = 'auto';
            else ul.style.width = this.dom.offsetWidth + 'px';
...
AutoCompleteSelector.prototype = {
    createDefaultSelectorElement: function() {
        ...
        var _top = target.offsetHeight + target.offsetTop - 1;
...
AutoCompleteSelectItem.prototype = {
    get_index: function(obj) {
        for (var i = 0; i < this.autoCompleteSelectorObject.dom.childNodes.length; i++) {
            if (this.autoCompleteSelectorObject.dom.childNodes[i] == obj) return i;
        }
        return - 1;
    },
    create: function() {
        ...
        li.onmouseover = function(evt) {
            ...
            if (div_index == -1) return __this.autoCompleteSelectorObject.selectItem(div_index);
