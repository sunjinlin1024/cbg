function fix_buy_btn() {
    ...
    if (IsLogin) {
        ...
        $("buy_btn").addEvent("click",
        function() {
            var isEquipInTestServer = test_server_list.contains(equip['server_id']);
            var isUserInTestServer = test_server_list.contains(parseInt(ServerInfo["server_id"]));
            ...
            if (equip['server_id'] != parseInt(ServerInfo["server_id"]) && isEquipInTestServer && !isUserInTestServer) {
                var ret = confirm('1. 您的角色所在服务器和该商品所在服务器属于不同的测试服版本，完成购买后商品暂时无法取出（藏宝阁有未取出商品时无法转服）。\n\n2. 每周二测试服更换后才能尝试取出，您确定要下单购买吗？');
                if (ret !== true) {
                    return false;
                }
            }
            $(document.equip_info).grab(new Element('input', {
                'type': 'hidden',
                'name': 'safe_code',
                'value': window.SafeCode
            })).grab(new Element('input', {
                'type': 'hidden',
                'name': 'view_loc',
                'value': window.equip_refer_loc
            })).submit();
            return false;
        });
    } else {
...
function add_events_for_collect(equip) {
    ...
    if (!IsLogin) {
        ...
        function() {
            ...
            if (getPara('view_loc') && getPara('view_loc') == 'search_remen') {
                equip.collect_refer = 'search_remen';
            }
...
function show_buy_tips() {
    ...
    if (equip["storage_type"] == StorageStype["money"]) {
