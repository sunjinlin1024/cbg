...
var SumAttrs = [['physique', '体质'], ['endurance', '耐力'], ['dex', '敏捷'], ['magic', '魔力'], ['power', '力量']];
var ServerTypes = [[3, '3年以上服'], [2, '1到3年服'], [1, '1年内服']];
var FairShowStatus = [[1, '已上架'], [0, '公示期']];
var FrontStatus = [['pass_fair_show', '已上架'], ['fair_show', '公示期']];
var Gems = [[1, '红玛瑙'], [2, '太阳石'], [3, '舍利子'], [4, '光芒石'], [5, '月亮石'], [6, '黑宝石'], [7, '神秘石'], [12, '翡翠石']];
var ProduceFroms = [[1, '系统产出'], [2, '人造']];
var EquipAttrs160 = [[1, '物理暴击几率'], [2, '法术暴击几率'], [3, '物理暴击伤害'], [4, '法术暴击伤害'], [5, '治疗能力'], [6, '封印命中率'], [7, '抵抗封印命中率'], [8, '穿刺效果'], [9, '格挡物理伤害'], [10, '魔法回复'], [11, '法术伤害减免效果']];
var OverallEquipSearcher = new Class({
    ...
    init_reset_btn: function() {
        var __this = this;
        $('reset_equip_attr').addEvent('click',
        function() {
            __this.reset_equip_attr();
        });
        $('reset_server_info').addEvent('click',
        function() {
            __this.reset_server_info();
        });
        $('reset_all').addEvent('click',
        function() {
            __this.reset_equip_attr();
            __this.reset_server_info();
        });
    ...
    reset_equip_attr: function() {
        ...
        var txt_inputs = [$('txt_init_damage'), $('txt_init_damage_raw'), $('txt_init_defense'), $('txt_init_hp'), $('txt_init_dex'), $('txt_init_wakan'), $('txt_all_damage'), $('txt_damage'), $('txt_sum_attr_value'), $('txt_gem_level'), $('txt_hole_num'), $('txt_repair_fail'), $('txt_price_min'), $('txt_price_max'), $('txt_added_status'), $('txt_append_skill'), $('txt_transform_skill'), $('txt_transform_charm')];
    ...
    reg_event: function() {
        var __this = this;
        $('btn_all_special_skill').addEvent('click',
        function() {
        ...
        $$('#check_mode_panel input').addEvent('click',
        function() {
    ...
    init_search_btn: function() {
        var __this = this;
        $('btn_equip_search').addEvent('click',
        function() {
    ...
    init_role_search_event: function() {
        var __this = this;
        $('link_search_related_role').addEvent('click',
        function() {
    ...
    search: function(func) {
        ...
        var txt_int_items = [['init_damage', 0, 10000, '初伤（包含命中）'], ['init_damage_raw', 0, 10000, '初伤（不含命中）'], ['all_damage', 0, 10000, '总伤'], ['damage', 0, 10000, '伤害'], ['init_defense', 0, 10000, '初防'], ['init_hp', 0, 10000, '初血'], ['init_dex', 0, 10000, '初敏'], ['init_wakan', 0, 10000, '初灵'], ['sum_attr_value', 0, 10000, '属性总和'], ['price_min', 0, MaxTradeYuan, '价格'], ['price_max', 0, MaxTradeYuan, '价格'], ['gem_level', 0, 20, '宝石锻炼等级'], ['hole_num', 0, 5, '装备开孔数目'], ['repair_fail', 0, 5, '修理失败次数']];
        var intReg = /^\d+$/;
        ...
        if ($('for_role_race').value) arg['for_role_race'] = $('for_role_race').value;
        if ($('for_role_sex').value) arg['for_role_sex'] = $('for_role_sex').value;
        var $160_attr = $('160_attr');
        if ($160_attr.value) {
            arg['160_attr'] = $160_attr.value;
        }
        if ($('chk_filter_hun_da_gem').checked) arg['filter_hun_da_gem'] = 1;
        if ($('chk_attr_with_melt').checked) {
            var attrInput = ['init_damage', 'init_damage_raw', 'all_damage', 'damage', 'init_defense', 'init_hp', 'init_dex', 'init_wakan'];
...
var SuitInputAutor = new Class({
    initialize: function(tagid, values, text, getter) {
        ...
        this.init();
        var that = this;
        $('radio_' + this.tagid).addEvent('click',
        function() {
            that.getter.chose(that);
        });
        $('txt_' + this.tagid).addEvent('click',
        function() {
            that.getter.chose(that);
        });
    },
...
var SuitSelectAutor = new Class({
    initialize: function(tagid, getter) {
        ...
        $('sel_' + this.tagid).addEvent('click',
        function() {
            that.getter.chose(that);
        });
    },
            ...
            },
            {
                ...
                get_val: function() {
                    ...
                    if (arg.hasOwnProperty("gem_level")) {
                        desc.push("宝石锻炼等级≥" + arg["sum_attr_value"]);
                    }
...
var HotSearch = new Class({
    ...
    bindEvent: function(labels) {
        var ctx = this;
        $('overall_hot_search_panel').addEvent('click:relay(.jsHotSearchTips)',
        function() {
