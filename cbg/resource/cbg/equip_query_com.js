...
function show_kind_tree(hide_kinds) {
    render_to_replace("kind_tree_panel", "kind_tree_templ", {
...
function init_sub_nav() {
    var NavManager = function(dom) {
        ...
        if (SearchType == SearchTypes["role_search"]) {} else if (SearchType == SearchTypes["pet_search"]) {} else if (SearchType == SearchTypes["equip_search"]) {} else if (SearchType == SearchTypes['offsale_search']) {
            this.item_index = 2;
        } else if (SearchType == SearchTypes['appointed_search']) {} else if (SearchType == SearchTypes['seller_search']) {} else {
            this.item_index = 0;
        }
...
function get_kind_info_by_name_list(name_list) {
    ...
    for (var i = 0; i < name_list.length; i++) {
        if (kind_id_obj[name_list[i]]) {
            kind_list.push({
                "kind_name": name_list[i],
                "kind_id": kind_id_obj[name_list[i]]
            });
        }
    }
    return kind_list;
...
function search_pet(page) {
    ...
    var txt_int_items = [['price_min', 0, MaxTradeYuan, '价格'], ['price_max', 0, MaxTradeYuan, '价格'], ['aptitude', 0, 5000, '召唤兽资质']];
...
function search_lingshi(page) {
    ...
    var txt_int_items = [['price_min', 0, MaxTradeYuan, '价格'], ['price_max', 0, MaxTradeYuan, '价格'], ['jinglian_level', 0, 16, '精练等级'], ['basic_attr_value', 0, 5000, '基础属性']];
...
function get_price_arg(prefix) {
    var arg = {};
    var txt_int_items = [['price_min', 0, MaxTradeYuan, '价格'], ['price_max', 0, MaxTradeYuan, '价格']];
...
function search_equip(page, act) {
    ...
    if (kind == 'search_role_equip') {
        ...
        var args_config = [["speed", "速度"], ["fangyu", "防御"], ["mofa", "魔法"], ["shanghai", "伤害"], ["hp", "气血"], ["xiang_qian_level", "宝石"], ["addon_sum_min", "属性总和"], ["addon_minjie_reduce", "敏捷减少"]];
        ...
        if (arg['s_type'] == '612') {
            arg['keyword'] = $('sel_guidebook_zhizao').value;
        } else if (arg['s_type'] == '18126') {
            arg['keyword'] = $('sel_guidebook_lingshi').value;
        } else if (arg['s_type'] == '6904') {
            arg['keyword'] = $('sel_lingxiyu_effect').value;
        } else {
...
function search_role(page) {
    ...
    var txt_int_items = [['level_min', 0, 175, '等级'], ['level_max', 0, 175, '等级'], ['price_min', 0, MaxTradeYuan, '价格'], ['price_max', 0, MaxTradeYuan, '价格'], ['sum_exp_min', 0, 3000000, '总经验'], ['sum_exp_max', 0, 3000000, '总经验'], ['bb_expt', 0, 100, '宠物修炼'], ['qian_yuan_dan', 0, 200, '乾元丹'], ['school_skill_level', 0, 200, '师门技能等级'], ['qian_neng_guo', 0, 500, '潜能果数']];
    var int_args = check_txt_int_arg(txt_int_items);
    if (int_args == null) {
        return;
    }
    Object.append(arg, int_args) if (arg['price_min'] > arg['price_max']) {
        alert('最低价格不能大于最高价格');
        return;
    }
    if (arg['price_min'] == 0) {
...
function init_advance_search_box() {
    ...
    var handler = function(keyword) {
        ...
        for (var i = 0; i < name_list.length; i++) {
            ...
            if (ename.indexOf(keyword) != -1) result.push(ename);
        }
        return result;
    };
...
function set_equip_search_event() {
    $('sel_material_s_type') && $('sel_material_s_type').addEvent('change',
    function() {
    ...
    $$('#advance_search_panel .label_role_equip').addEvent('focus',
    function() {
        set_radio_check('equip_kind', 'search_role_equip');
    });
    $$('#advance_search_panel .label_pet_equip').addEvent('focus',
    function() {
    ...
    $$('#advance_search_panel .label_material').addEvent('focus',
    function() {
        set_radio_check('equip_kind', 'search_material');
    });
    $$('#advance_search_panel .label_stone').addEvent('focus',
    function() {
        set_radio_check('equip_kind', 'search_stone');
    });
    $$('#advance_search_panel .label_jl_stone').addEvent('focus',
    function() {
        set_radio_check('equip_kind', 'search_jl_stone');
    });
    $$('#advance_search_panel .label_msyj_neidan').addEvent('focus',
    function() {
...
function if_need_display_highlight() {
    ...
    if (["search_role_equip", "search_role_equip_no_refer", "search_pet", "search_role"].contains(act)) {
        return true;
    }
...
function() {
    if (if_need_display_highlight()) {
        $$('.jsGenHighlights').each(function($el) {
            ...
            $el.set({
                html: gen_highlight_html(highlights, "<br />", true),
                style: {
                    display: ''
                }
            });
