    ...
    for (var i = 0; i < config.length; i++) {
        ...
        try {
            ...
            case 'A':
                ...
                if (!v) {
                    prop = config[i][4];
                    if (prop) v = elem.getElement(key).getProperty(prop);
                    if (!v) v = '-';
                }
                break;
            case 'L':
...
cbg_tracer.send_log = function(log_name, attrs, callback) {
    ...
    var img = new Image();
    var params = {}; (attrs || []).each(function(list) {
...
cbg_tracer.trace_select_role = function() {
    ...
    window.login = function() {
        ...
        var config = [['ver', 'C_1'], ['ip', 'S_#client_ip'], ['fingerprint', 'F_Fingerprint'], ['urs', 'S_#user_urs'], ['serverid', 'V_serverid'], ['serversn', 'V_serversn'], ['roleid', 'C_' + roleid], ['nickname', 'C_' + role['nickname']], ['icon', 'C_' + role['icon']], ['race', 'C_' + role['racename']]];
        var attrs = cbg_tracer.get_attrs(config);
        Cookie.write('fingerprint', cbg_tracer.fingerprint.get(), {
            'duration': 1
        });
        cbg_tracer.send_log('select_role', attrs,
        function() {
            old_login();
        });
    };
...
cbg_tracer.get_common_trace_info = function() {
    ...
    var config = [['client_type', 'C_web'], ['fingerprint', 'F_Fingerprint'], ['urs', 'C_' + urs], ['roleid', 'C_' + roleid], ['nickname', 'C_' + nickname], ['icon', 'C_' + icon], ['grade', 'C_' + user_level], ['school', 'C_' + school], ['nav', 'S_.nav ul li.on'], ['sub_nav', 'S_.subNav a.on'], ['is_user_login', 'C_' + is_login]];
    if (window.LoginInfo) {
        config.extend([['serverid', 'E_LoginInfo.serverid'], ['serversn', 'E_LoginInfo.serversn']]);
    } else {
        config.extend([['serverid', 'E_ServerInfo.server_id'], ['serversn', 'E_ServerInfo.server_sn']]);
    }
    return config;
}
...
cbg_tracer.trace_search = function() {
    var config = cbg_tracer.get_common_trace_info() config.extend([['ver', 'C_1'], ['kind', 'S_#kind_tree_panel ul .on:not(.off)']]);
...
cbg_tracer.get_overall_search_conds = function() {
    ...
    if (window.QueryArgs) {
        ...
        if (CurSearchValue.order_by && CurSearchValue.order_by['key'] != null) {
            search_conds['query_order'] = CurSearchValue.order_by['key'] + ' ' + CurSearchValue.order_by['sort'];
        }
...
cbg_tracer.do_trace_overall_search = function() {
    var config = cbg_tracer.get_common_trace_info() config.extend([['ver', 'C_1'], ['kind', 'C_-']]);
...
cbg_tracer.trace_show_equip_detail = function() {
    ...
    if ($root.length > 0) {
        ...
        $root.each(function($r) {
            ...
            $r.set(keyInit, 1);
            $r.addEvent('click:relay(#soldList a.soldImg,#soldList a.jsStat,.jsStatEquips a.soldImg,.jsStatEquips a.jsStat)',
            function(e) {
                var res = cbg_tracer.do_trace_show_equip_detail($(this).getParent('tr'));
                if (res === false) {
                    e.stop();
                }
            });
            $r.addEvent('mouseenter:relay(#soldList a.soldImg,.jsStatEquips a.soldImg)',
            function() {
                $(this).setProperty('mouse_over_begin', new Date());
            });
            $r.addEvent('mouseleave:relay(#soldList a.soldImg,.jsStatEquips a.soldImg)',
            function() {
                var begin = $(this).getProperty('mouse_over_begin');
                begin = new Date(begin);
                var end = new Date();
                var delta = end - begin;
                cbg_tracer.do_trace_show_tips_box($(this).getParent('tr'), delta);
            });
        });
    } else {
...
cbg_tracer.trace_special_equip_lists = function() {
    $$('#msg_detail .j_equip_icon a.soldImg').addEvents({
        ...
        mouseleave: function() {
            ...
            var end = new Date();
            var delta = end - begin;
            cbg_tracer.do_trace_show_tips_box($(this).getParent('.j_equip_icon'), delta);
        },
        click: function() {
            return cbg_tracer.do_trace_show_equip_detail($(this).getParent('.j_equip_icon'));
        }
    });
    var panels = $$('#recent_list_panel, #latest_trades_list');
    if (panels.length == 0) return;
    panels.addEvent('click',
    function(e) {
        var img = e.target;
    ...
    $$('.j_link_order_detail_to_equip').addEvent('click',
    function(e) {
...
cbg_tracer.get_common_info_for_equip_in_list = function(e) {
    ...
    try {
        ...
        try {
            ...
            var search_conds = JSON.encode(cbg_tracer.get_overall_search_conds());
            config.extend([['equip_serverid', 'A_a img', e, 'data_serverid']]);
            config.extend([['equip_serversn', 'A_a img', e, 'data_serversn']]);
        } catch(exp2) {
            extra = {};
            var search_conds = '-';
            config.extend([['equip_serverid', 'A_a img', e, 'data_serverid']]);
    ...
    config.extend([['ver', 'C_1'], ['view_loc', 'C_' + view_loc], ['kind', 'S_#kind_tree_panel ul .on:not(.off)'], ['search_conds', 'C_' + search_conds], ['ordersn', 'A_a img', e, 'data_ordersn'], ['equip_type', 'A_a img', e, 'data_equip_type'], ['equip_level', 'A_a img', e, 'data_equip_level'], ['selling_time', 'A_a img', e, 'data_selling_time'], ['create_time', 'A_a img', e, 'data_equip_create_time', 'data_create_time'], ['kindid', 'A_a img', e, 'data_kindid'], ['price', 'A_a img', e, 'data_equip_price', 'data_price'], ['highlight', 'C_' + (extra.highlight ? extra.highlight: '-')], ['status', 'A_a img', e, 'data_equip_status', 'data_status'], ['pos', 'C_' + pos]]);
    if (!extra.seller_roleid) {
        config.extend([['seller_roleid', 'A_a img', e, 'data_seller_roleid', 'data_owner_roleid']]);
    } else {
        config.extend([['seller_roleid', 'C_' + (extra.seller_roleid ? extra.seller_roleid: '-')]]);
    }
    var attrs = cbg_tracer.get_attrs(config);
    for (var i = 0; i < attrs.length; i++) {
        ...
        d = new Date(val);
        attrs[i][1] = d.getFullYear() + '-' + (d.getMonth() + 1) + '-' + d.getDate() + ' ' + d.getHours() + ':' + d.getMinutes() + ':' + d.getSeconds()
    }
    return attrs;
}
cbg_tracer.do_trace_show_equip_detail_on_link = function(e, view_loc) {
    var config = cbg_tracer.get_common_trace_info();
    config.extend([['equip_serverid', 'E_ServerInfo.server_id'], ['equip_serversn', 'E_ServerInfo.server_sn'], ['ver', 'C_1'], ['view_loc', 'C_' + view_loc], ['kind', 'C_-'], ['search_conds', 'C_-'], ['ordersn', 'A', e, 'data_ordersn'], ['equip_type', 'A', e, 'data_equip_type'], ['equip_level', 'A', e, 'data_equip_level'], ['selling_time', 'A', e, 'data_selling_time'], ['create_time', 'A', e, 'data_equip_create_time', 'data_create_time'], ['kindid', 'A', e, 'data_kindid'], ['price', 'A', e, 'data_equip_price', 'data_price'], ['highlight', 'C_-'], ['status', 'A', e, 'data_equip_status', 'data_status'], ['pos', 'C_-'], ['seller_roleid', 'A', e, 'data_seller_roleid', 'data_owner_roleid']]);
    var attrs = cbg_tracer.get_attrs(config);
    for (var i = 0; i < attrs.length; i++) {
        var key = attrs[i][0];
        var val = attrs[i][1];
...
cbg_tracer.do_trace_show_equip_detail = function(e) {
    var a_elem = e.getElement('a.soldImg') || e.getElement('a img').getParent('a');
...
cbg_tracer.trace_pay_cross_server_order = function() {
    if (window.goto_pay_orders) {
        ...
        window.goto_pay_orders = function() {
            var config = cbg_tracer.get_common_trace_info();
            var e = $$('#soldList')[0];
            config.extend([['ver', 'C_1'], ['type', 'C_' + trace_log_name], ['price', 'A_a img', e, 'data_equip_price'], ['equip_serverid', 'E_equip_info.serverid'], ['equipid', 'A_a img', e, 'data_equipid']]);
            var attrs = cbg_tracer.get_attrs(config);
...
cbg_tracer.trace_merge_pay = function() {
    if (window.do_pay) {
        ...
        window.do_pay = function() {
            var config = cbg_tracer.get_common_trace_info();
            var e = $$('#soldList')[0];
            config.extend([['ver', 'C_1'], ['type', 'C_' + trace_log_name], ['price', 'L_a img', e, 'data_equip_price'], ['equip_serverid', 'E_ServerInfo.server_id'], ['equipid', 'L_a img', e, 'data_equipid']]);
            var attrs = cbg_tracer.get_attrs(config);
            cbg_tracer.send_log('goto_epay', attrs);
...
cbg_tracer.trace_pay_in_order_list = function() {
    if (window.go_pay) {
        ...
        window.go_pay = function(orderid) {
            var config = cbg_tracer.get_common_trace_info();
            var e = $$('#soldList')[0];
            config.extend([['ver', 'C_1'], ['type', 'C_' + trace_log_name], ['price', 'A_#equip_img_' + orderid, e, 'data_equip_price'], ['equip_serverid', 'E_ServerInfo.server_id'], ['equipid', 'A_#equip_img_' + orderid, e, 'data_realequipid']]);
            var attrs = cbg_tracer.get_attrs(config);
            cbg_tracer.send_log('goto_epay', attrs);
...
cbg_tracer.trace_pay_in_detail_page = function() {
    if (window.go_pay) {
        ...
        window.go_pay = function() {
            var config = cbg_tracer.get_common_trace_info();
            var e = $$('#soldList')[0];
            config.extend([['ver', 'C_1'], ['type', 'C_' + trace_log_name], ['price', 'V_price'], ['equip_serverid', 'E_ServerInfo.server_id'], ['equipid', 'E_document.equip_info.equipid.value']]);
            var attrs = cbg_tracer.get_attrs(config);
            cbg_tracer.send_log('goto_epay', attrs);
            return old_go_pay.apply(this, arguments);
        }
...
cbg_tracer.trace_in_equip_detail_page = function() {
    ...
    if (location.search.match(/equip_refer=56|_web_log=1/)) {} else if (/\/login\.py.*act=do_anon_auth/.test(referrer)) {
        referrer = '';
    ...
    if (window.equip_refer_loc) {
        ...
        window.equip_refer_loc = view_loc = 'link_external';
    }
    var config = cbg_tracer.get_common_trace_info();
    config.extend([['highlight', 'C_' + (equip.highlights ? JSON.encode(equip.highlights) : '-')], ['ver', 'C_1'], ['view_loc', 'C_' + view_loc], ['ordersn', 'E_equip.game_ordersn'], ['equip_type', 'E_equip.equip_type'], ['selling_time', 'E_equip.selling_time'], ['create_time', 'E_equip.create_time'], ['kindid', 'E_equip.kindid'], ['seller_roleid', 'E_equip.owner_roleid'], ['price', 'E_equip.price'], ['equip_serversn', 'E_equip.serversn'], ['equip_level', 'E_equip.equip_level'], ['status', 'E_equip.status']]);
    var attrs = cbg_tracer.get_attrs(config);
...
cbg_tracer.trace_recommd_in_equip_detail_page = function($root) {
    ...
    if ($list.length > 0) {
        ...
        $list.addEvent('click',
        function() {
            cbg_tracer.do_trace_recommd_in_equip_detail_page($(this).getParent('li'));
            return true;
        });
        $list.addEvent('mouseover',
        function() {
            $(this).setProperty('mouse_over_begin', new Date());
        });
        $list.addEvent('mouseout',
        function() {
            var begin = $(this).getProperty('mouse_over_begin');
            if (!begin) {
                return;
            ...
            var end = new Date();
            var delta = end - begin;
            cbg_tracer.do_trace_show_tips_box_in_equip_detail_page($(this).getParent('li'), delta);
        });
    } else {
...
cbg_tracer.do_trace_recommd_in_equip_detail_page = function(e) {
    var config = cbg_tracer.get_common_trace_info();
    config.extend([['highlight', 'A_a img', e, 'data_highlights'], ['ver', 'C_1'], ['kind', 'S_#kind_tree_panel ul .on:not(.off)'], ['tag', 'A_a img', e, 'data_recommd_tag'], ['type', 'C_from_detail_page'], ['view_loc', 'A_a img', e, 'data_view_loc'], ['ordersn', 'A_a img', e, 'data_ordersn'], ['equip_type', 'A_a img', e, 'data_equip_type'], ['equip_level', 'A_a img', e, 'data_equip_level'], ['selling_time', 'A_a img', e, 'data_selling_time'], ['create_time', 'A_a img', e, 'data_create_time'], ['kindid', 'A_a img', e, 'data_kindid'], ['seller_roleid', 'A_a img', e, 'data_owner_roleid'], ['price', 'A_a img', e, 'data_price'], ['equip_serversn', 'A_a img', e, 'data_serversn'], ['status', 'A_a img', e, 'data_status'], ['ref_equip_serversn', 'E_equip.serversn'], ['ref_ordersn', 'E_equip.game_ordersn'], ['pos', 'C_' + cbg_tracer.get_list_pos(e)]]);
    var attrs = cbg_tracer.get_attrs(config);
    cbg_tracer.send_log('show_equip_detail', attrs);
}
cbg_tracer.do_trace_show_tips_box_in_equip_detail_page = function(e, delta) {
    ...
    if (delta < 500 || storage_type == StorageStype['money'] || storage_type == StorageStype['role']) {
        return;
    }
    var config = cbg_tracer.get_common_trace_info();
    config.extend([['highlight', 'A_a img', e, 'data_highlights'], ['ver', 'C_1'], ['kind', 'S_#kind_tree_panel ul .on:not(.off)'], ['tag', 'A_a img', e, 'data_recommd_tag'], ['type', 'C_from_detail_page'], ['view_loc', 'A_a img', e, 'data_view_loc'], ['ordersn', 'A_a img', e, 'data_ordersn'], ['equip_type', 'A_a img', e, 'data_equip_type'], ['equip_level', 'A_a img', e, 'data_equip_level'], ['selling_time', 'A_a img', e, 'data_selling_time'], ['create_time', 'A_a img', e, 'data_create_time'], ['kindid', 'A_a img', e, 'data_kindid'], ['seller_roleid', 'A_a img', e, 'data_owner_roleid'], ['price', 'A_a img', e, 'data_price'], ['equip_serversn', 'A_a img', e, 'data_serversn'], ['status', 'A_a img', e, 'data_status'], ['duration', 'C_' + delta], ['ref_equip_serversn', 'E_equip.serversn'], ['ref_ordersn', 'E_equip.game_ordersn'], ['pos', 'C_' + cbg_tracer.get_list_pos(e)]]);
    var attrs = cbg_tracer.get_attrs(config);
