...
var ROLE_ZHUAN_ZHI_CONFIG = {
    ...
    ctx[name] = defination();
})(window, 'RoleInfoParser',
function() {
    function each(obj, callback) {
        if (typeOf(obj) === 'array') {
            for (var i = 0,
            max = obj.length; i < max; i++) {
    ...
    RoleInfoParser.prototype = {
        ...
        parse_basic_role_info: function() {
            ...
            each(this.raw_info.idbid_desc || [],
            function(id) {
                if (id >= 5 && id <= 8) role_info['is_gold_id'] = true;
            });
        ...
        parse_role_skill: function() {
            ...
            for (var skill in raw_skill_info) {
                ...
                if (conf["life_skill"][skill]) {
                    ...
                    info["skill_pos"] = conf["school_skill"][skill]["pos"] school_skill.push(info);
        ...
        parse_pet_info: function() {
            ...
            var get_ending_name = function(itype) {
                return that.conf.ending_info[itype];
            };
            ...
            var get_pet_info = function(pet, is_child) {
                ...
                if (is_child) {
                    if (pet.hasOwnProperty('isnew') && pet.hasOwnProperty('isnew') == 1) {
                        ...
                        info["ending"] = get_ending_name(pet["ending"]);
                        info["gg"] = pet["gg"];
                        info["zl"] = pet["zl"];
                        info["wl"] = pet["wl"];
                        info["dl"] = pet["dl"];
        ...
        parse_single_prop_kept: function(prop, grade) {
            ...
            var attr_list = [];
            for (var key in prop) {
                if (this.PROP_KEPT_KEYS[key] && prop[key] >= (grade * 2 + 10)) attr_list.push({
                    key: key,
                    value: prop[key],
                    name: this.PROP_KEPT_KEYS[key]
                });
            }
            if (attr_list.length < 1) return null;
            if (attr_list.length < 2) return attr_list[0].name;
        ...
        make_img_name: function(img_name) {
            var img_id = parseInt(img_name) var addon = "";
            if (img_id < 10) {
                addon = "000";
