    ...
    function request(url, params, opts) {
        ...
        function(data) {
            ...
            if (!data) {
                ...
                var length = (data.equips || []).length || 0;
                if (length < params.count) {
                    wlog('RECOMMAND_COUNT_ERROR', ['tag=' + (data.equips && data.equips[0] && data.equips[0].tag || ''), 'count=' + length + '/' + params.count, 'url=' + url, 'params=' + JSON.encode(params)].join('|'));
                }
            }
        });
        req.send({
...
function init_recommands() {
    ...
    function requestAndInit($pt) {
        ...
        var callback = window[$pt.getAttribute('data_callback')] ||
        function() {};
