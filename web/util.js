...
function setSelect(selobj, value) {
    for (var i = 0; i < selobj.length; i++) {
        if (selobj.options[i].value == value || selobj.options[i].value + "省" == value || selobj.options[i].value + "区" == value || selobj.options[i].value + "市" == value) selobj.options[i].selected = true;
...
function set_select_by_text(sel_obj, target_text, sublength) {
    ...
    for (var i = 0; i < sel_obj.length; i++) {
        ...
        option_text = option_text.substr(0, sublength) if (target_text == option_text) {
...
var Template = new Class({
    ...
    compile: function() {
        var start = 0;
        var delimeter = '_%_';
        var body = this.template.replace(this.options.tag_re,
...
function check_offsale_equips() {
    ...
    if (!offsale_equips_num) return;
    offsale_equips_num = offsale_equips_num.toInt();
    if (offsale_equips_num <= 0) return;
    Cookie.write("remind_offsale", 1, {
        'duration': 4
    });
...
function fix_anonymous_menu_link() {
    ...
    if ($('top_fairshow_menu_a')) $("top_fairshow_menu_a").href += '&server_id=' + ServerInfo["server_id"];
    if ($('sub_offsale_query_a')) $("sub_offsale_query_a").href += '&server_id=' + ServerInfo["server_id"];
}
...
InstalmentStatus[ORDER_INSTALMENT_FORFEITED_PENDING] = "定金扣除";
InstalmentStatus[ORDER_INSTALMENT_FORFEITED_FINISH] = "定金扣除";
...
function adjust_tips_position(el, tips_box, fixes) {
    ...
    if (fixes) {
        if (fixes['top']) styles['top'] += fixes['top'];
        if (fixes['left']) styles['left'] += fixes['left'];
    }
...
function fix_buy_menu_url() {
    ...
    if ($("common_query_a")) $("common_query_a").addEvent("click", goto_query_page);
}
function add_latest_view(serverid, equipid) {
    ...
    if (pos1 != -1) {
        ...
        if (sp.length >= 20) equipids = equipids.substring(equipids.indexOf('-') + 1);
    }
    if (equipids != '') equipids += '-' + new_one;
    else equipids += new_one;
...
function show_wallet_guide() {
    ...
    if (WalletData.is_locked) {
        if (Cookie.read("wallet_tips") != WELLET_LOCKED_TIPS) {
            tipsContent = '您的钱包已被锁定，请<a href="' + WalletData.customer_service_url + '" target="_blank" class="cPink" style="margin-left:2px;">联系客服</a>';
    ...
    if (tipsContent && target) {
        ...
        el_guide.getElement("button").addEvent("click",
        function() {
...
function init_get_money() {
    ...
    function initEvents() {
        $("get_money_count").addEvent("keyup",
        function() {
        ...
        $("submit_get_money").addEvent("click",
        function() {
    ...
    function showConfrim(amount) {
        ...
        confirmDialog.show();
        $('btn_confrim_get_money').addEvent('click',
        function() {
            confirmDialog.hide();
            submit(amount);
        });
    ...
    btn_get_money && btn_get_money.addEvent("click",
    function() {
...
function init_buy_ecard() {
    ...
    function initEvents() {
        ...
        function() {
            if (this.value == '其它点数') {
                ...
                if (this.value != num) {
                    ...
                    this.value = num;
                    this.setSelectionRange && this.setSelectionRange(start, end);
                }
            }
        });
        $otherNum.addEvent('blur',
        function() {
            setTimeout(function() {
        ...
        $root.addEvent('click:relay(.submit)',
        function() {
            if (!config.mobile) {
                dialog.hide();
                showBindPhoneDialog();
    ...
    function showConfrim() {
        var tmpl = ['<div style="padding:24px 56px;">', '<p style="margin:6px 0;">手机号：', config.mobile, '</p>', '<p style="white-space:nowrap;">', '<input class="txt1 sms_code" type="text" placeholder="请输入短信验证码" /> ', '<input class="txt1 btn_get_code" type="button" value="获取验证码" style="min-width:100px;text-align:center;color:#333;background:#efefef;" />', '</p>', '<p id="sms_verify_tips" class="cRed tips" style="margin:6px 0;">&nbsp;</p>', '<p class="textCenter">', '<a class="btn1 btn_submit">确认支付</a>', '</p>', '</div>', '<div class="divider" style="margin:0 10px;"></div>', '<div style="margin:10px;line-height:1.5;" class="cDGray">', config.isUrsMobile ? '当前手机号为网易关联手机，支付成功后将绑定藏宝阁；<br>': '', '如需更换手机号，请前往我的藏宝阁<a href="' + CgiRootUrl + '/bind_phone.py?act=show_bind_phone" target="_blank">手机服务</a>', '</div>'].join('');
        var countDownTimer;
        ...
        $btnGetSmsCode.addEvent('click',
        function() {
            if (sms_code_time_limit > 0) return;
                    ...
                    if (result.status == AjaxConstants.Ok) {
                        if (sms_code_time_limit <= 0) {
                            ...
                            countDownTimer = setInterval(function() {
                                if (sms_code_time_limit > 0) {
                                    sms_code_time_limit--;
                                    $btnGetSmsCode.value = sms_code_time_limit + '秒后重新发送';
                                } else {
                                    clearInterval(countDownTimer);
        ...
        $root.getElement('.sms_code').addEvent('keyup',
        function(event) {
            if (event.key == 'enter') {
    ...
    function showSuccessDialog(orderId) {
        var orderDetailUrl = 'http://ecard.163.com/script/pay_success?order_id=' + orderId;
        new InfoDialog({
            title: '支付成功',
            type: InfoDialog.TYPE.SUCCESS,
            info: ['已向点卡中心下单，可到点卡中心查询详细<br>订单信息', '（单号：<a href="', orderDetailUrl, '" target="_blank">', orderId, '</a>）。'].join(''),
            buttons: [{
                name: '查看钱包记录',
        ...
        new Request.JSON({
            ...
            onSuccess: function(result) {
                if (result.status == AjaxConstants.Ok) {
                    ...
                    if (!result.ecard_act) {
                        var $specialType = $root.getElement('.checkItem[data-type="4"]');
                        if ($specialType.hasClass('currItem')) {
                            buyType = config.defaultType;
                            $root.getElement('.checkItem[data-type="' + buyType + '"]').addClass('currItem');
                        }
                        $specialType.dispose();
                    }
    ...
    function init() {
        ...
        $root.getElement('.ecardType .checkItem[data-type="' + buyType + '"]').addClass('currItem');
        $root.getElement('.ecardNumber .checkItem[data-num="' + buyNum + '"]').addClass('currItem');
        if ($otherNum.hasClass('currItem')) {
...
function init_right_float_menu() {
    ...
    $("latest_view_btn").addEvent("click",
    function() {
        if (!latest_view_open) {
    ...
    if (el_customer_service_link && customer_service_url && customer_service_url.indexOf('<' + '!--') == -1) {
        if (!is_user_login()) {
            el_customer_service_link.addEvent('click',
            function() {
                alert_login();
            });
        } else {
...
function init_ad() {
    ...
    function show_ad_top(data) {
        ...
        $wrapper.getElement('.cbg-tgxc-close').addEvent('click',
        function() {
            $wrapper.destroy();
    ...
    function show_ad_pop(data) {
        ...
        $wrapper.addEvent('click',
        function() {
            StoreDB.setItem(flag, 1);
        });
        $wrapper.inject(document.body);
    }
    function is_show_by_tags(tags, blacks, whites) {
        tags = tags || [];
        var hasBlack = blacks && blacks.length > 0;
        var hasWhite = whites && whites.length > 0;
        var isAdShow = true;
        if (hasBlack) {
            for (var i = 0,
            ilen = tags.length; i < ilen; i++) {
                if (blacks.contains(tags[i])) {
                    return false;
                }
            }
            isAdShow = true;
    ...
    function render_ad() {
        ...
        for (var pos in conf) {
            ...
            data = data.filter(function(item) {
                return is_show_by_tags(userTags, item.disable_user_tags || [], item.display_user_tags || []);
            });
    ...
    if (is_user_login() && roleid) {
        var sessionAdId = 'sad_tag_' + roleid;
        var sessionData = SessionDB.getItem(sessionAdId);
        var fnSuccess = function(data) {
            if (data.status === 1) {
                userTags = data.result;
                SessionDB.setItem(sessionAdId, JSON.encode(data.result || []));
            }
            render_ad();
        };
        if (sessionData) {
            try {
                userTags = JSON.decode(sessionData);
            } catch(e) {} finally {
                render_ad();
            }
        } else {
            var req = new Request.JSON({
                url: CgiRootUrl + '/query.py?act=user_tags_query',
                noCache: true,
...
Popup.prototype = {
    ...
    show: function() {
        ...
        set_position_center(this.pop_el) var __this = this;
...
var PopupDialog = new Class({
    ...
    setup: function() {
        var that = this;
        that.getRoot().set("html", this.getTemplate()).getElement(".dialogClose").addEvent("click",
        function() {
            that.hide();
        });
    },
    ...
    getTemplate: function() {
        var tmpl = ['<div class="dialogWrap">', '<div class="blockTitle">', this._title, '<span class="dialogClose"></span>', '</div>', '<div class="dialogCont">', this._content, '</div>', '</div>'];
        return tmpl.join('');
    },
    showMask: function() {
        if (!this.el_mask) {
...
PopupDialog.alert = function(content, opts) {
    ...
    function generateButtons($root) {
        ...
        if (buttons.length > 0) {
            ...
            buttons.forEach(function(btn, i) {
                ...
                list.push('<a href="javascript:;" ' + (btn.close ? ' data-close="1" ': '') + ' class="' + (btn.cls ? btn.cls: '') + ' btn1 ' + defCls + '">' + btn.text + '</a>');
                if (btn.onclick) {
            ...
            html = '<a href="javascript:;" class="btn1" data-close="1">' + buttonText + '</a>';
        }
    ...
    $dialog.addEvent('click:relay([data-close])',
    function() {
        popup.hide();
    });
...
function switch_select(ctrl_box_id, el_list_id) {
    ...
    var el_list = $$(el_list_id) for (var i = 0; i < el_list.length; i++) {
        el_list[i].checked = v;
        el_list[i].setAttribute("checked", v);
    }
}
function cascade_checkbox(el_main, el_sub, callback) {
    el_main.addEvent('click',
    function() {
        el_sub.set('checked', this.checked) callback && callback.call(this);
    });
    el_sub.addEvent('click',
    function() {
        var isChecked = true;
...
var BargainPanel = (function() {
    ...
    function setHoverEvent($root, selector, options) {
        ...
        options || {});
        $root.addEvent('mouseenter:relay(' + selector + ')',
        function(e) {
            options.enter.call(this, e);
        }).addEvent('mouseleave:relay(' + selector + ')',
        function(e) {
    ...
    function check_price(price_yuan) {
        if (!/^\d+$/.test(price_yuan)) return "您填写的价格有错误"
        var lim = BargainLimits[1];
        var price = Math.round(price_yuan * 100);
        if (price >= lim.cur_price) return "还价价格要低于原价，请重新输入！";
        if (price < Math.round(lim.cur_price * 0.65)) return "低于可还价范围";
        if (price <= lim.last_bargain_price) return '还价价格必须高于上次还价价格(' + (lim.last_bargain_price / 100) + '元)';
        if (price >= lim.last_rebargain_price) return '还价价格高于卖家还价价格，请直接购买';
        if (price < lim.equip_min_price) return '还价价格不能低于该物品允许的最低价格(' + (lim.equip_min_price / 100) + '元)';
    ...
    function check_price_level(price_yuan) {
        ...
        else if (price < cur_price * 0.9) return {
            text: '一般',
            color: 'orange'
        };
        else return {
    ...
    function update_msg() {
        ...
        if (window.LoginInfo && LoginInfo.login) from_server_name = LoginInfo.server_name;
        var msg = $("bargain_msg_templ").value.replace(/<!--bargain_price_desc-->/g, htmlEncode(price_info)).replace(/\n+/g, '<br/>');
...
var BargainResponsePanel = (function() {
    ...
    function init(loc) {
        ...
        $('btn_accept_bargain').addEvent('click',
        function() {
            bargain_response('accept');
        });
        $('btn_refuse_bargain').addEvent('click',
        function() {
            bargain_response('refuse');
        });
        $('btn_rebargain').addEvent('click',
        function() {
        ...
        $('btn_malicious_bargain').addEvent('click',
        function() {
            var radios = $$('input[name=maliciousBargainRadio]:checked');
            var opt = radios && radios[0] && radios[0].get('value');
            if (opt == 'ignore') bargain_response('refuse');
            else if (opt == 'forbidden') bargain_response('malicious');
            else alert('请选择期望的处理方式');
        });
    }
...
var InputLimit = {
    _limit: function($input, selector, fnIsPass) {
        var lastValue = '';
    ...
    onlyNumber: function($input, selector) {
    ...
    onlyInt: function($input, selector) {
        this._limit($input, selector,
        function isPass(value) {
            return /^\d*$/.test(value);
        });
...
function desc_4_mhb(amount) {
    ...
    if (ten_million > 0) msg += ten_million + '千';
    million = parseInt(amount / 1000000) % 10;
    if (million > 0) msg += million + '百';
    if (!ten_million) {
        hundred_thousand = parseInt(amount / 100000) % 10;
        if (hundred_thousand > 0) msg += hundred_thousand + '十';
    }
    return '约' + msg + '万';
}
...
var AjaxGuard = (function() {
    ...
    var CaptchaPopup = new Class({
        initialize: function(opts) {
            this.opts = Object.merge({
                onSuccess: null
            },
            opts || {});
            var popup = this.popup = new PopupDialog('验证', '<div style="width:250px;padding:15px 30px;">\
          ...
          <ul>\
            ...
            var $close = popup.dialog.getElement('.dialogClose');
            $close && $close.dispose();
            this.bindEvent();
            popup.show();
        },
        bindEvent: function() {
            var self = this;
            var $root = this.popup.dialog;
            var $image = $root.getElement('.validate_img');
            var $value = $root.getElement('.validate_value');
            function changeImage() {
                $image.set('src', CgiRootUrl + "/create_validate_image.py?act=search_captcha&stamp=" + Math.random());
            }
            $root.getElement('.validate_img').addEvent('click', changeImage);
            $root.getElement('.validate_change').addEvent('click', changeImage);
            $root.getElement('.validate_button').addEvent('click',
            function() {
                var captcha_value = $value.get('value').trim();
                ...
                var Ajax = new Request.JSON({
                    ...
                    onSuccess: function(result) {
                        if (result.status == 0) {
                            self.hide();
                            self.opts.onSuccess && self.opts.onSuccess();
                        } else {
                            alert(result.msg);
                            changeImage();
                        }
