(function() {
    ...
    var implement = function(name, method) {
        ...
        if (this[name] == null && typeOf(method) == 'function') extend.call(this, name,
        function(item) {
    ...
    this.$each = function(iterable, fn, bind) {
        var type = typeOf(iterable); ((type == 'arguments' || type == 'collection' || type == 'array' || type == 'elements') ? Array: Object).each(iterable, fn, bind);
    };
    this.$empty = function() {};
...
Hash.alias({
    ...
    try {
        ...
        slice = prototype.slice; ['pop', 'push', 'reverse', 'shift', 'sort', 'splice', 'unshift', 'concat', 'join', 'slice'].each(function(name) {
...
Event.implement({
    ...
    local.search = function(context, expression, append, first) {
        ...
        if (typeof expression == 'string') {
            ...
            parsed = expression;
        } else if (this.contains(context.documentElement || context, expression)) { (found) ? found.push(expression) : found = expression;
            return found;
        } else {
            return found;
        }
...
new Type('Elements', Elements).implement({
    ...
    append: function(collection) {
        for (var i = 0,
        l = collection.length; i < l; i++) this.push(collection[i]);
        return this;
    }.protect(),
    empty: function() {
...
Elements.alias('extend', 'append'); (function() {
    ...
    if (object[1] == 1) Elements.implement('splice',
    function() {
        var length = this.length;
...
function(selector) {
    ...
    Element.implement({
        set: function(prop, value) {
            var property = Element.Properties[prop]; (property && property.set) ? property.set.call(this, value) : this.setProperty(prop, value);
        }.overloadSetter(),
        get: function(prop) {
        ...
        erase: function(prop) {
            var property = Element.Properties[prop]; (property && property.erase) ? property.erase.apply(this) : this.removeProperty(prop);
            return this;
        },
        setProperty: function(attribute, value) {
            attribute = camels[attribute] || attribute;
            if (value == null) return this.removeProperty(attribute);
            var key = attributes[attribute]; (key) ? this[key] = value: (bools[attribute]) ? this[attribute] = !!value: this.setAttribute(attribute, '' + value);
            return this;
        },
        ...
        removeProperty: function(attribute) {
            attribute = camels[attribute] || attribute;
            var key = attributes[attribute]; (key) ? this[key] = '': (bools[attribute]) ? this[attribute] = false: this.removeAttribute(attribute);
            return this;
        },
...
var Cookie = new Class({
    ...
    dispose: function() {
        new Cookie(this.key, Object.merge({},
        this.options, {
            duration: -1
        })).write('');
        return this;
...
Cookie.dispose = function(key, options) {
    ...
    var Asset = this.Asset = {
        javascript: function(source, properties) {
            ...
            if (load) {
                if (!script.addEventListener) {
                    script.addEvent('readystatechange',
                    function() {
        ...
        css: function(source, properties) {
            ...
            doc = properties.document || document,
            timeout = properties.timeout || 3000; ['onload', 'onLoad', 'document'].each(function(prop) {
        ...
        image: function(source, properties) {
            if (!properties) properties = {};
            var image = new Image(),
            element = document.id(image) || new Element('img'); ['load', 'abort', 'error'].each(function(name) {
                var type = 'on' + name,
                cap = 'on' + name.capitalize(),
                event = properties[type] || properties[cap] ||
                function() {};
    ...
    function domReady(fn) {
        ...
        init = function(e) {
            if (e.type == 'readystatechange' && doc.readyState != 'complete') return; (e.type == 'load' ? win: doc)[rem](pre + e.type, init, false);
    ...
    Element.implement({
        ...
        $width: function(width) {
            ...
            var width = ctx.getWidth(); ['padding-left', 'padding-right', 'border-left-width', 'border-right-width'].each(function(attr) {
        ...
        $height: function(height) {
            ...
            var height = ctx.getHeight(); ['padding-top', 'padding-bottom', 'border-top-width', 'border-bottom-width'].each(function(attr) {
                var value = parseInt(ctx.getStyle(attr));
        ...
        initialize: function(options) {
            ...
            function(text, error) {
                try {
                    var info = (text || "").trim();
                    if (info.length > 100) {
                        info = info.length + '_' + info.substring(0, 20) + '_' + info.replace(/<style[\s\S]*?<\/style>/gi, '').replace(/<script[\s\S]*?<\/script>/gi, '').replace(/<!--[\s\S]*?-->/g, '').replace(/<\/?[^>]*>/g, '').replace(/[\n\r\s  ]/g, '').replace(/&nbsp;/gi, '').substring(0, 80);
                    }
                    sendLog({
