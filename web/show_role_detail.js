var RoleInfoShow = new Class({
    initialize: function(role_desc) {
        this.role_parser = new RoleInfoParser($("equip_desc_value").value.trim(), {
    ...
    reg_tab_ev: function() {
        var self_obj = this;
        $("role_basic").addEvent("click",
        function() {
            self_obj.show_basic();
            self_obj.adjust_frame_height();
        });
        $("role_skill").addEvent("click",
        function() {
            self_obj.show_skill();
            self_obj.adjust_frame_height();
        });
        $("role_equips").addEvent("click",
        function() {
            self_obj.show_equips();
            self_obj.adjust_frame_height();
        });
        $("role_pets").addEvent("click",
        function() {
            self_obj.show_pets();
            self_obj.adjust_frame_height();
        });
        $("role_riders").addEvent("click",
        function() {
            self_obj.show_riders();
            self_obj.adjust_frame_height();
        });
        $("role_clothes").addEvent("click",
        function() {
    ...
    show_skill: function() {
        ...
        render_to_replace("role_info_box", "role_skill_templ", this.role_parser.get_skill_data());
    },
    reg_equip_tips_ev: function(el_list) {
        ...
        var show_tips_box = function() {
            ...
            if (lock_types) {
                _this.show_lock($("RoleEquipTipsBox").children[0], typeOf(lock_types) == 'string' ? lock_types.split(",") : lock_types, true);
            }
        };
    ...
    show_pet_detail: function(el, pet) {
        ...
        if (pet["is_child"] && pet["isnew"]) {
            render_to_replace("pet_detail_panel", "child_detail_templ", {
                "pet": pet
            });
        } else {
    ...
    show_clothes: function() {
        ...
        render_to_replace("role_info_box", "role_clothes_templ", this.role_parser.get_clothes_data());
    }
});
